...
TXT_greet_1 = "Доброго вечера! Я - чат-бот, созданный, чтобы помочь с выбором настольной игры. Начинаем!"
TXT_greet_2 = f"""Позвольте представиться надлежащим образом...
Я владею архивом, небольшой базой данных из {games_num} игр. Буду искать в ней настолки, которые больше подходят именно вам. Для этого мне придется немного пораспрашивать вас.

Давайте условимся, что команда /begin заставит меня начать нашу беседу и мои поиски.
Команда /help даст понять, что у вас возникли проблемы, и я постараюсь помочь.
Если вы захотите побольше узнать о моей игротеке, то воспользуйтесь командой /games
А также мы можем пройти в мой скромный архив и я поведую вам подробнее про любую из игр /describe"""
...
TXT_tags_help = '''Выберите то, что обязательно должно присутствовать в вашей ыигре. Можете выбрать из предложенного, можете написать в 1 сообщении несколько вариантов, либо выбрать "нет предпочтений". Если найдутся игры, удовлетворяющие всем вашим пожеланиям, я их и выберу. Если точного совпадения не будет - я покажу то, что подбирал для вас до этого.
...
Викторины - это игры, в которых нужно отвечать на вопросы, проверяющие ваш кругозор и знания.
Подвижные игры могут включать такие игровые элементы, как, например, пантомима.
Социальные - игры, в которых вам придется взаимодействовать с другими игроками, общаться с ними.
Карточные игры основаны на характеристиках карт и чаще всего включают большой элемент случайности.
Командные - в них все игроки в 1 команде страраются на благо общей победы.
...
TXT_not_best = 'Как так, в моей неидеальной базе нет идеальной игры для вас, но посоветовать все же кое-что я могу. Но у вас отличный вкус, вот что я думаю!\n"Еще" или плюсик в чат, и мы продолжим. А для перезапуска есть команда /begin'
TXT_help_next = 'Нажмайте "далее", чтобы открывать больше информации обо мне. Команда /begin вернет меня к началу нашей беседы.'
TXT_help = "У вас есть какие-то конкретные вопросы или вы хотите посмотреть всю документацию целиком? Если готовы начать - нажмите /begin"
TXT_help_full = [TXT_party_help, TXT_time_help, TXT_lvl_help, TXT_tags_help, TXT_yes_help]
TXT_archive = f"В моей игротеке целых {games_num} игр! Вот они, слева направо:"
TXT_describe = "Какая настольная игра вас интересует? Если же вам снова понадобится мой совет - я всегда к вашим услугам /begin"
TXT_describe_ = "На этом здесь придется закончить... Пройдемте в мой скромный архив?"
TXT_describe_err = "Такой игры, к сожалению, у меня в базе нет... Ознакомиться со списком, кстати, можно через команду /games"
...
def main():
    ...
    h_ind = None # для отображения полной документации
    ...
    @dp.message_handler(commands=['help'], state = BotStates.helping_state)
    @dp.message_handler(commands=['help'], state = BotStates.choosing_state)
    @dp.message_handler(commands=['help'], state = BotStates.describing_state)
    ...
    async def helping(msg: types.Message):
        ...
        if msg.text.lower() == 'далее':
            try:
                await bot.send_message(chat_id=msg.chat.id, text=next(h_ind))
            except:
                h_ind = None
    ...
    @dp.message_handler(commands=['describe'], state = '*')
    async def describing(msg: types.Message):
        """
        Начало команды `/describe`
        """
        keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True) 
        buttons = ['/begin', '/games', '/help']
        keyboard.add(*buttons) 
        await bot.send_message(chat_id=msg.chat.id, text=TXT_describe_)
        await msg.answer(TXT_describe, reply_markup = keyboard)
        await BotStates.describing_state.set()
    ...
    @dp.message_handler(state=BotStates.describing_state)
    async def show_ann(msg: types.Message):
        """
        Обработчик команды `/describe`
        """
        keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
        game = df[df.name.map(lambda x: x.lower() == msg.text.lower())]
        if not game.empty:
            txt = 'Описание гласит:\n\n"' + game['annotation'].to_list()[0] + '"'
            await msg.answer(txt, reply_markup = keyboard)
        else:
            await msg.answer(TXT_describe_err, reply_markup = keyboard)
